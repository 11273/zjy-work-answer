...
class OAuthLoginHandler:
    ...
    def find_available_port(self, preferred_port=None, max_attempts=50):
        ...
        common_ports = (
            list(range(39001, 39050))
            + list(range(38000, 38050))
            + list(range(37000, 37050))
        )
    ...
    def start_server(self, retry_count=3):
        ...
        for attempt in range(retry_count):
            try:
                ...
                logger.warning(
                    f"⚠️ 启动OAuth登录服务器失败 (尝试第{attempt + 1}次): {e}"
                )
